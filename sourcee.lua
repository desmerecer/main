...
rares.Ascii = "qwertyuiopasdfghjklzxcvbnmQWRTYUIOPASDFGHJKLZXCVBNM";
rares.GLOBAL_ENVIRONMENT = {};
rares.Windows = {};
rares.FontSemiBold = Font.new('rbxasset://fonts/families/GothamSSm.json',Enum.FontWeight.SemiBold,Enum.FontStyle.Normal);
rares.Flags = {};
rares.Colors = {
    Black = Color3.fromRGB(16, 16, 16),
    Main  = Color3.fromRGB(183, 28, 28),     -- vermelho escuro forte (#B71C1C)
    Accent = Color3.fromRGB(211, 47, 47),    -- vermelho médio pra highlights (#D32F2F)
    TextHighlight = Color3.fromRGB(229, 57, 53)  -- pro stroke/texto de destaque
};
...
function rares:IsMobile() : boolean
	return UserInputService.TouchEnabled;	
end;

function rares:RandomString() : string
...
function rares:GetTextSize(Text : TextLabel,CustomFont: Enum.Font) : Vector2
...
function rares:IsMouseOverFrame(Frame : Frame) : boolean
...
function rares:GetCalculatePosition(planePos: number, planeNormal: Vector3, rayOrigin: number, rayDirection: Vector3) : number
...
function rares:CreateHover(Element : Frame,Callback : (boolean) -> any)
...
function rares:GetIcon(name : string) : string
	return rares.Lucide['lucide-'..tostring(name)] or rares.Lucide[name] or rares.Lucide[tostring(name)] or "";
end;

function rares:SetIcon(name : string, value : string)
	rares.Lucide[name] = value;
end;

function rares:Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function rares:CreateAnimation(Instance: Instance , Time: number , Style : Enum.EasingStyle , Property : {[string] : any}) : Tween
...
function rares:NewInput(Frame : Frame , Callback : () -> ()) : TextButton
...
function rares:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		rares:CreateAnimation(MoveFrame,Speed,nil,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #rares.DragBlacklist <= 0 then 
			dragToggle = true
	...
	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #rares.DragBlacklist <= 0 then
			if dragToggle then
				updateInput(input)
			end
		else
			if #rares.DragBlacklist > 0 then
				dragToggle = false
			end
		end
	end);
...
function rares:ScrollSignal(Scroll: ScrollingFrame,UIListLayout: UIListLayout,Type:string)
	if Type == 'X' then
...
function rares:CreateResponse(args: {[string] : (any) -> any})
	local main = {};
...
function rares:GetWindowFromElement(Element: GuiObject)
	for i,v in next , rares.Windows do
...
function rares:CreateOption(OptionButton: ImageButton): Elements
	rares:CreateHover(OptionButton,function(bool)
		if bool then
			rares:CreateAnimation(OptionButton,0.5,{
	...
	local OwnWindow = rares:GetWindowFromElement(OptionButton);
	...
	rares:ScrollSignal(ScrollingFrame,UIListLayout,'Y');

	ExtElementFrame.Active = true;
	ExtElementFrame.Name = rares:RandomString()
	...
	ExtElementFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	DropShadow.Name = rares:RandomString()
	...
	local ToggleExt = function(bool)
		if bool then
			...
			rares:CreateAnimation(ExtElementFrame,0.45,{
				Size = UDim2.new(0, 200, 0, size)
			})

			rares:CreateAnimation(DropShadow,0.45,{
				ImageTransparency = 0.750
			})

			rares:CreateAnimation(UIStroke,0.45,{
				Transparency = 0
			})
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait(0.1)
					...
					rares:CreateAnimation(ExtElementFrame,0.35,{
						Position = ud
					});
				end;
			end)
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
			if not rares:IsMouseOverFrame(ExtElementFrame) and not rares.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER then
				ToggleExt(false);
			end;
		end;
	end);

	local UIElements: Elements = rares:CreateElements(ScrollingFrame,ScrollingFrame.ZIndex,Bindable);

	return UIElements;
end;

function rares:CreateColorPicker(ColorBox: Frame,Transparency, Callback)
	Transparency = Transparency or 0;
	Callback = Callback or function() end;

	local OwnWindow = rares:GetWindowFromElement(ColorBox);
	...
	local VisibleToggle = function(value)
		if value then
			...
			rares:CreateAnimation(ColorPickBox,0.45,{
				ImageTransparency = 0,
				BackgroundTransparency = 0
			});

			rares:CreateAnimation(MouseMovement,0.45,{
				ImageTransparency = 0,
			});

			rares:CreateAnimation(UIStroke_2,0.45,{
				Transparency = 0
			});

			rares:CreateAnimation(ColorRedGreenBlue,0.45,{
				BackgroundTransparency = 0
			});

			rares:CreateAnimation(ColorRGBSlide,0.45,{
				BackgroundTransparency = 0
			});

			rares:CreateAnimation(UIStroke_3,0.45,{
				Transparency = 0.75
			});

			rares:CreateAnimation(ColorOpc,0.45,{
				BackgroundTransparency = 0
			});

			rares:CreateAnimation(ColorOptSlide,0.45,{
				BackgroundTransparency = 0
			});

			rares:CreateAnimation(UIStroke_4,0.45,{
				Transparency = 0.75
			});

			rares:CreateAnimation(UIStroke_5,0.45,{
				Transparency = 0
			});

			rares:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.45
			});

			rares:CreateAnimation(CopyButton,0.45,{
				ImageTransparency = 0.45
			});

			rares:CreateAnimation(hexCode,0.45,{
				BackgroundTransparency = 0.4
			});

			rares:CreateAnimation(HexCodeText,0.45,{
	...
	ColorPickerFrame.Name = rares:RandomString()
	...
	rares:AddDragBlacklist(ColorPickerFrame);
	...
	MouseMovement.Name = rares:RandomString()
	...
	MouseMovement.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	ColorRedGreenBlue.Name = rares:RandomString()
	...
	ColorRedGreenBlue.BorderColor3 = Color3.fromRGB(255, 255, 255)
...
UIGradient.Color = ColorSequence.new{
    ...
    ColorSequenceKeypoint.new(0.20, Color3.fromRGB(255, 111, 0)),   -- laranja/vermelho
    ColorSequenceKeypoint.new(0.30, Color3.fromRGB(211, 47, 47)),
    ColorSequenceKeypoint.new(0.40, Color3.fromRGB(183, 28, 28)),
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(136, 14, 14)),
    -- ... continua com tons de vermelho/laranja escuro
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(183, 28, 28))
}
...
UIGradient.Parent = ColorRedGreenBlue
	...
	ColorRGBSlide.Name = rares:RandomString()
	...
	ColorRGBSlide.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	ColorOptSlide.Name = rares:RandomString()
	...
	ColorOptSlide.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	ColorOpt.Name = rares:RandomString()
	...
	ColorOpt.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	PasteButton.Name = rares:RandomString()
	...
	PasteButton.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	CopyButton.Name = rares:RandomString()
	...
	hexCode.Name = rares:RandomString()
	hexCode.Parent = ColorOpt
	hexCode.BackgroundColor3 = rares.Colors.Black
	hexCode.BackgroundTransparency = 0.400
	hexCode.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	HexCodeText.Name = rares:RandomString()
	...
	HexCodeText.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	HexCodeText.FontFace = rares.FontSemiBold
	...
	rares:CreateHover(CopyButton,function(bool)
		if bool then
			rares:CreateAnimation(CopyButton,0.45,{
	...
	rares:CreateHover(PasteButton,function(bool)
		if bool then
			rares:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.1
			})
	...
	PasteButton.MouseButton1Click:Connect(function()
		if rares.GLOBAL_ENVIRONMENT.COLOR_COPY then
			local h,s,v = rares.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHSV();

			Transparency = rares.GLOBAL_ENVIRONMENT.COLOR_COPY.OPC;

			OldCode = h;
			CodeH = s;
			CodeV = v;

			HexCodeText.Text = "#" .. tostring(rares.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHex());
			...
			ColorOpc.BackgroundColor3 = rares.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB;
	...
	CopyButton.MouseButton1Click:Connect(function()
		rares.GLOBAL_ENVIRONMENT.COLOR_COPY = {
	...
	rares:NewInput(ColorBox,function()
		VisibleToggle(true);
	end);
	...
	do
		...
		ColorPickerFrame.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				rares.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = true;
		...
		ColorPickerFrame.InputEnded:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				IsPressM1 = false;
				rares.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = false;
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if not rares:IsMouseOverFrame(ColorPickerFrame) then
					VisibleToggle(false);
				end;
			end;
		end)
		...
		ColorRedGreenBlue.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait()
					...
					rares:CreateAnimation(ColorBox,0.5,{
						BackgroundColor3 = Color
					});

					rares:CreateAnimation(ColorOpc,0.35,{
						BackgroundColor3 = Color
					});

					rares:CreateAnimation(ColorPickBox,0.5,{
		...
		ColorPickBox.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait();
					...
					rares:CreateAnimation(MouseMovement,0.2,nil,{
						Position = UDim2.new(CodeH, 0, 1 - CodeV, 0)
					})
				end
			end
...
function rares:RandomStr(len: number): string
	local main = "";

	for i = 1 , len do
		local max = #rares.Ascii;
		local rand = math.random(1,max);

		main = main .. rares.Ascii:sub(rand,rand);
	end;

	return main;
end;

function rares:AddDragBlacklist(Frame: Frame)
	Frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			local finder = table.find(rares.DragBlacklist , Frame);

			if not finder then
				table.insert(rares.DragBlacklist , Frame);
			end;
		end;
	end);
	...
	Frame.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if finder then
				table.remove(rares.DragBlacklist , finder);
			end;
		end;
	end);
end;

function rares:ProtectText(Label: TextLabel,Text: string)
	Label.RichText = true;

	local MainText = string.gsub(Text,'.',function(t)
		if not string.find(t,"[<>()\"']") then
			return string.format('<font %s="%s">%s</font>',rares:RandomStr(5),rares:RandomStr(5),t)
		end;
		return t;
	end);
...
function rares:CreateDropdown(Parent: Frame, Default: string | {[string]: boolean}, Multiplier: boolean, AutoUpdate: boolean,Callback: (data: any) -> any)
	local Window = rares:GetWindowFromElement(Parent);
	local Data = {};
	...
	rares:AddDragBlacklist(DropdownItemFrame);
		...
		if value then
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait()
					...
					rares:CreateAnimation(DropdownItemFrame,0.35,{
						Size = UDim2.new(0, 175, 0, math.clamp(baseSize,0,200))
					})
				end;
			end)

			rares:CreateAnimation(UIStroke,0.35,{
				Transparency = 0
			})

			rares:CreateAnimation(DropShadow,0.35,{
				ImageTransparency = 0.75
			})
		else
			if SPAWN_THREAD then
	...
	DropdownItemFrame.Name = rares:RandomString()
	...
	DropdownItemFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	local func;
	local res = rares:CreateResponse({
		set_data = function(v)
			Data = v;
		end,
			...
			for i,v in next , Data do
				...
				if Multiplier then
					if (typeof(Default) == 'table' and (Default[v] or table.find(Default,v))) or Default == v then
						Selected[v] = true
						bth.TextColor3 = rares.Colors.Main;
					else
	...
	rares:NewInput(Parent,function()
		if AutoUpdate then
			res:refresh();
		end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not rares:IsMouseOverFrame(DropdownItemFrame) then
				func(false);
				Toggle(false);
			end;
		end;
...
function rares:CreateElements(Parent : Frame , ZIndex : number , Event : BindableEvent,SearchAPI: {Path: string,Memory : (name:string) -> any}) : Elements
	local elements = {};
	local FatalWindow = rares:GetWindowFromElement(Parent);
	...
	function elements:AddToggle(Config: Toggle)
		...
		Toggle.Name = rares:RandomString()
		...
		rares:AddDragBlacklist(Toggle);

		Toggle_Name.Name = rares:RandomString()
		...
		Toggle_Name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		Toggle_Name.FontFace = rares.FontSemiBold;
		Toggle_Name.TextColor3 = (Config.Risky and Color3.fromRGB(255, 255, 255)) or Color3.fromRGB(255, 255, 255)
		Toggle_Name.TextSize = 13.000
		Toggle_Name.TextTransparency = 1
		Toggle_Name.TextXAlignment = Enum.TextXAlignment.Left;

		rares:ProtectText(Toggle_Name,Config.Name);

		ValueFrame.Name = rares:RandomString()
		ValueFrame.Parent = Toggle
		ValueFrame.AnchorPoint = Vector2.new(1, 0.5)
		ValueFrame.BackgroundColor3 = rares.Colors.Black
		ValueFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		ValueIcon.Name = rares:RandomString()
		...
		ValueIcon.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		OptionButton.Name = rares:RandomString()
		...
		local toggleImg = function(value)
			if value then
				rares:CreateAnimation(ValueIcon,0.45,{
					ImageTransparency = 0,
					ImageColor3 = rares.Colors.Main,
		...
		local OpcToggle = function(value)
			if value then
				...
				rares:CreateAnimation(OptionButton,0.45,{
					ImageTransparency = (Config.Option and 0.600) or 1
				})

				toggleImg(Config.Default)

				rares:CreateAnimation(ValueFrame,0.45,{
					BackgroundTransparency = 0,
				})

				rares:CreateAnimation(Toggle_Name,0.45,{
		...
		rares:CreateHover(ValueFrame,function(b)
		...
		rares:NewInput(ValueFrame,function()
			Config.Default = not Config.Default;
			toggleImg(Config.Default);
			Config.Callback(Config.Default)
		end);

		local Respons = rares:CreateResponse({
			Rename = function(new_name)
				Toggle_Name.Text = new_name;
				rares:ProtectText(Toggle_Name,new_name);
			end,
			GetValue = function()
			...
			Option = (Config.Option and rares:CreateOption(OptionButton)) or nil;
		});

		if Config.Flag then
			rares.WindowFlags[FatalWindow][Config.Flag.."Toggle"] = Respons;
		end;
	...
	function elements:AddSlider(Config: Slider)
		...
		Slider.Name = rares:RandomString()
		...
		rares:AddDragBlacklist(Slider);

		Slider_Name.Name = rares:RandomString()
		...
		Slider_Name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		Slider_Name.FontFace = rares.FontSemiBold
		Slider_Name.Text = Config.Name
		Slider_Name.TextColor3 = (Config.Risky and Color3.fromRGB(255, 255, 255)) or Color3.fromRGB(255, 255, 255)
		...
		boxli.Name = rares:RandomString()
		boxli.Parent = ValueFrame
		boxli.BackgroundColor3 = rares.Colors.Main
		boxli.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		ValueText.Name = rares:RandomString()
		...
		ValueText.FontFace = rares.FontSemiBold
		...
		local OpcToggle = function(value)
			if value then
				rares:CreateAnimation(boxli,0.45,{
					BackgroundTransparency = 0,
				})
				...
				rares:CreateAnimation(Slider_Name,0.45,{
					TextTransparency = 0.200
				})

				rares:CreateAnimation(ValueText,0.45,{
		...
		local function update(Input)
			...
			local Value = rares:Rounding(Main,Config.Round);
			...
			ValueFrame.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not rares:IsMouseOverFrame(ValueFrame) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		if Config.Flag then
			rares.WindowFlags[FatalWindow][Config.Flag.."Slider"] = Respons;
		end;
	...
	function elements:AddButton(Config: Button)
		...
		Button.Name = rares:RandomString()
		Button.Parent = Parent
		Button.BackgroundColor3 = rares.Colors.Black
		Button.BorderColor3 = Color3.fromRGB(255, 255, 255)
		Button.BorderSizePixel = 0
		Button.Size = UDim2.new(1, -25, 0, 25)
		Button.ZIndex = ZIndex + 1
		rares:AddDragBlacklist(Button);

		Button_Name.Name = rares:RandomString()
		...
		Button_Name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		Button_Name.FontFace = rares.FontSemiBold
		Button_Name.Text = Config.Name
		Button_Name.TextColor3 = (Config.Risky and Color3.fromRGB(255, 255, 255)) or Color3.fromRGB(255, 255, 255)
		Button_Name.TextSize = 12.000
		Button_Name.TextTransparency = 0.400
		rares:ProtectText(Button_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				rares:CreateAnimation(Button_Name,0.45,{
					TextTransparency = 0.4,
				})

				rares:CreateAnimation(Button,0.45,{
					BackgroundTransparency = 0,
				})
			else
				rares:CreateAnimation(Button_Name,0.45,{
		...
		rares:CreateHover(Button,function(value)
			if value then
				rares:CreateAnimation(Button,0.45,{
				...
				rares:CreateAnimation(Button,0.45,{
					BackgroundColor3 = rares.Colors.Black
				})
			end;
		end)

		rares:NewInput(Button,function()
			Config.Callback();
		end)

		return rares:CreateResponse({
			Rename = function(new_name)
				Button_Name.Text = new_name
				rares:ProtectText(Button_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
	...
	function elements:AddColorPicker(Config: ColorPicker)
		...
		ColorPicker.Name = rares:RandomString()
		...
		rares:AddDragBlacklist(ColorPicker);

		ColorPicker_Name.Name = rares:RandomString()
		...
		ColorPicker_Name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		ColorPicker_Name.FontFace = rares.FontSemiBold
		...
		local res = rares:CreateColorPicker(ValueFrame,Config.Transparency,function(rgb,opc)
		...
		local OpcToggle = function(value)
			if value then
				rares:CreateAnimation(ColorPicker_Name,0.45,{
		...
		if Config.Flag then
			rares.WindowFlags[FatalWindow][Config.Flag.."ColorPicker"] = Respons;
		end;
	...
	function elements:AddDropdown(Config: Dropdown)
		...
		Dropdown.Name = rares:RandomString()
		...
		rares:AddDragBlacklist(Dropdown);

		Dropdown_Name.Name = rares:RandomString()
		...
		Dropdown_Name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		Dropdown_Name.FontFace = rares.FontSemiBold
		...
		rares:ProtectText(Dropdown_Name,Config.Name);
		...
		Value_Text.Name = rares:RandomString()
		...
		Value_Text.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		Value_Text.FontFace = rares.FontSemiBold
		...
		local res;
		res = rares:CreateDropdown(ValueFrame,Config.Default,Config.Multi,Config.AutoUpdate,function(args)
			Config.Default = args;
			Value_Text.Text = DataParser(Config.Default);
		...
		res:on_toggle(function(b)
			if b then
				rares:CreateAnimation(icon,0.35,{
					Rotation = -180,
					ImageColor3 = rares.Colors.Main
				})
			else
		...
		local OpcToggle = function(value)
			if value then
				rares:CreateAnimation(Dropdown_Name,0.45,{
					TextTransparency = 0.2,
				})
				...
				rares:CreateAnimation(icon,0.45,{
					ImageTransparency = 0
				})

				rares:CreateAnimation(Value_Text,0.45,{
		...
		local Respons = rares:CreateResponse({
			Rename = function(new_name)
				...
				rares:ProtectText(Dropdown_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			rares.WindowFlags[FatalWindow][Config.Flag.."Dropdown"] = Respons;
		end;
	...
	function elements:AddKeybind(Config: Keybind)
		...
		Keybind.Name = rares:RandomString()
		...
		rares:AddDragBlacklist(Keybind);

		Keybind_Name.Name = rares:RandomString()
		...
		Keybind_Name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		Keybind_Name.FontFace = rares.FontSemiBold
		...
		rares:ProtectText(Keybind_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				rares:CreateAnimation(Keybind_Name,0.45,{
					TextTransparency = 0.2,
				})
		...
		local Respons = rares:CreateResponse({
			Rename = function(new_name)
				...
				rares:ProtectText(Keybind_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			...
			rares.WindowFlags[FatalWindow][Config.Flag.."Keybind"] = Respons;
		end;
...
function rares:CreateConfigWindow(Root: ScreenGui , Fatal , Button: ImageButton)
	...
	rares:AddDragBlacklist(ConfigWindowFrame);
	...
	local ElementToggle = function(value)
		...
		if value then
			...
			rares:CreateAnimation(ConfigWindowFrame,0.3,{
				Size = UDim2.new(0, 185, 0, 195),
			})

			rares:CreateAnimation(UIStroke,0.3,{
				Transparency = 0
			})

			rares:CreateAnimation(DropShadow,0.3,{
				ImageTransparency = 0.750
			})

			rares:CreateAnimation(ScrollingFrame,0.3,{
				BackgroundTransparency = 0
			})

			rares:CreateAnimation(UIStroke_2,0.3,{
				Transparency = 0.750
			})

			rares:CreateAnimation(UIStroke_3,0.3,{
				Transparency = 0.750
			})

			rares:CreateAnimation(SpaceBox,0.3,{
				BackgroundTransparency = 0
			})

			rares:CreateAnimation(InputFrame,0.3,{
				BackgroundTransparency = 0
			})

			rares:CreateAnimation(TextBox,0.3,{
				TextTransparency = 0.5
			})

			rares:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			rares:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			rares:CreateAnimation(SaveConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			rares:CreateAnimation(DeleteButton,0.3,{
			...
			rares:CreateAnimation(ConfigWindowFrame,0.275,{
	...
	ConfigWindowFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	ScrollingFrame.Active = true
	ScrollingFrame.BackgroundColor3 = rares.Colors.Black
	ScrollingFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
	ScrollingFrame.BorderSizePixel = 0
	...
	InputFrame.BackgroundColor3 = rares.Colors.Black
	...
	TextBox.ZIndex = 209
	TextBox.FontFace = rares.FontSemiBold
	TextBox.Text = ""
	...
	AddConfigButton.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	GetConfigButton.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	DeleteButton.ImageColor3 = rares.Colors.Main
	...
	rares:CreateHover(DeleteButton,function(bool)
		if bool then
			rares:CreateAnimation(DeleteButton,0.3,{
				ImageTransparency = 0;
			})
	...
	rares:CreateHover(SaveConfigButton,function(bool)
		if bool then
			rares:CreateAnimation(SaveConfigButton,0.3,{
	...
	rares:CreateHover(GetConfigButton,function(bool)
		if bool then
			rares:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	rares:CreateHover(AddConfigButton,function(bool)
		if bool then
			rares:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	game:GetService('UserInputService').InputBegan:Connect(function(Input,Typing)
		if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
			if UIToggle and not rares:IsMouseOverFrame(ConfigWindowFrame) then
				ElementToggle(false);
			end;
		end;
	end);
	...
	res = rares:CreateResponse({
		...
		Refresh = function()
			...
			for i,v in next, Configs do
				...
				if tostring(import.Name) == tostring(v) or import.Name == v then
					...
					import.Name = v;
					import.Element.TextColor3 = rares.Colors.Main;
					button.Text = "  "..tostring(v);
				end;
				...
				button.MouseButton1Click:Connect(function()
					...
					import.Element = button;
					import.Name = v;
					button.TextColor3 = rares.Colors.Main;
					button.Text = "  "..tostring(v);
				end);
			end;
		end,
		...
		Init = function(Name: string,Folder: string)
			if not isfolder(Folder or "rares") then
				makefolder(Folder or "rares");	
			end;

			if not isfolder((Folder or "rares").."/Config") then
				makefolder((Folder or "rares").."/Config");	
			end;

			local cfgPath = (Folder or "rares").."/Config/"..tostring(Name);
...
function rares.new(Window: Window)
	Window = Window or {};
	...
	Fatal.Notifier = rares.__NOTIFIER_CACHE or rares:CreateNotifier();

	local rareswin = Instance.new("ScreenGui")
	...
	rares.WindowFlags[rareswin] = {};

	rares:ScrollSignal(tbc,UIListLayout,'X');
	...
	FatalFrame:GetPropertyChangedSignal("BackgroundTransparency"):Connect(function()
		if FatalFrame.BackgroundTransparency >= 0.99 then
			rareswin.Enabled = false;
		else
			rareswin.Enabled = true;
		end;
	end);
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			rares:CreateAnimation(FatalFrame,0.15,{
				Size = Window.Scale,
				BackgroundTransparency = 0,
			})

			rares:CreateAnimation(Header,0.5,{
				BackgroundTransparency = 0,
			})

			rares:CreateAnimation(HeaderLine,0.5,{
				BackgroundTransparency = 0,
			})

			rares:CreateAnimation(Bottom,0.5,{
				BackgroundTransparency = 0,
			})

			rares:CreateAnimation(HeaderLine_2,0.5,{
				BackgroundTransparency = 0,
			})

			rares:CreateAnimation(HeaderLineShadow,0.5,{
				BackgroundTransparency = 0.5,
			})

			rares:CreateAnimation(HeaderLineShadow_2,0.5,{
				BackgroundTransparency = 0.5,
			})

			rares:CreateAnimation(InfoButton,0.25,{
				ImageTransparency = 0.5
			})

			rares:CreateAnimation(SearchButton,0.25,{
				ImageTransparency = 0.5
			})

			rares:CreateAnimation(SaveButton,0.25,{
				ImageTransparency = 0.5
			})

			rares:CreateAnimation(HeaderText,0.35,{
				TextStrokeTransparency = 0.640,
				TextTransparency = 0
			})

			rares:CreateAnimation(UserIcon,0.45,{
				ImageTransparency = 0,
				Position = UDim2.new(1, -10,0.5, 0)
			})

			rares:CreateAnimation(User_name,0.35,{
				TextTransparency = 0,
				Position = UDim2.new(1, -40,0, 3)
			})

			rares:CreateAnimation(expire_days,0.5,{
				TextTransparency = 0,
			...
			rares:CreateAnimation(InfoButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(1, -5,0.5, 0)
			})

			rares:CreateAnimation(SearchButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(0,10,0.5, 0)
			})

			rares:CreateAnimation(SaveButton,0.1,Enum.EasingStyle.Back,{
			...
			table.clear(rares.DragBlacklist);

			for i,v in next , Fatal.Menus do
				v.ValueSelect(false);
			end;

			rares:CreateAnimation(SaveButton,0.35,{
				Position = UDim2.new(0,35,1, 20)
			})

			rares:CreateAnimation(SearchButton,0.35,{
				Position = UDim2.new(0,10,1, 20)
			})

			rares:CreateAnimation(InfoButton,0.35,{
			...
			rares:CreateAnimation(UserIcon,0.35,{
				ImageTransparency = 1,
				Position = UDim2.new(1, -10,0.5, 0)
			})
			...
			rares:CreateAnimation(expire_days,0.1,{
				TextTransparency = 1,
				Position = UDim2.new(1, -40,0, 16)
			})
			...
			rares:CreateAnimation(FatalFrame,0.75,{
				BackgroundTransparency = 1,
			})
	...
	rareswin.Name = rares:RandomString();
	rareswin.Parent = CoreGui;
	rareswin.ResetOnSpawn = false;
	rareswin.IgnoreGuiInset = true;
	rareswin.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	table.insert(rares.Windows,rareswin)

	protect_gui(rareswin);

	FatalFrame.Active = true;
	FatalFrame.Name = rares:RandomString()
	FatalFrame.Parent = rareswin
	FatalFrame.AnchorPoint = Vector2.new(0.5, 0)
	FatalFrame.BackgroundColor3 = Color3.fromRGB(19, 19, 19)
	FatalFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	Header.Name = rares:RandomString()
	...
	MenuButtonCont.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	UserProfle.Name = rares:RandomString()
	...
	UserIcon.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	User_name.Name = rares:RandomString()
	...
	User_name.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	expire_days.Name = rares:RandomString()
	...
	expire_days.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	HeaderLineShadow.Name = rares:RandomString()
	HeaderLineShadow.Parent = Header
	HeaderLineShadow.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	HeaderLineShadow.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	MenuFrame.Name = rares:RandomString()
	...
	Bottom.Name = rares:RandomString()
	...
	HeaderLine_2.Name = rares:RandomString()
	HeaderLine_2.Parent = Bottom
	HeaderLine_2.BackgroundColor3 = Color3.fromRGB(29, 29, 29)
	HeaderLine_2.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	HeaderLineShadow_2.Name = rares:RandomString()
	HeaderLineShadow_2.Parent = Bottom
	HeaderLineShadow_2.AnchorPoint = Vector2.new(0, 1)
	HeaderLineShadow_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	HeaderLineShadow_2.BackgroundTransparency = 0.500
	HeaderLineShadow_2.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	rares:Drag(FatalFrame,FatalFrame,0.1);
	...
	function Fatal:GetFlags()
		return rares.WindowFlags[rareswin];
	end;
	...
	function Fatal:AddConfig()
		...
		ConfigButton.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		rares:CreateHover(ConfigButton,function(bool)
			if bool then
				rares:CreateAnimation(ConfigButton,0.3,{
					ImageTransparency = 0.2500;
				})
			else
				rares:CreateAnimation(ConfigButton,0.3,{
		...
		return rares:CreateConfigWindow(rareswin,Fatal,ConfigButton);
	end;
	...
	function Fatal:LoadConfig(config)
		...
		for i,v in next , config do
			if i ~= "Info" then
				local Element = rares.WindowFlags[rareswin][i];
	...
	function Fatal:AddMenu(Menu : Menu)
		...
		MenuButton.Name = rares:RandomString()
		...
		Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
		Icon.ZIndex = 5
		Icon.Image = rares:GetIcon(Menu.Icon);
		Icon.ImageColor3 = rares.Colors.Main
		Icon.ScaleType = Enum.ScaleType.Crop

		UICorner_2.CornerRadius = UDim.new(1, 0)
		UICorner_2.Parent = Icon

		menu_name.Name = rares:RandomString()
		...
		menu_name.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		local text_size = rares:GetTextSize(menu_name);
		...
		Center.Name = rares:RandomString()
		Center.Parent = MenuLiber
		Center.Active = true
		Center.AnchorPoint = Vector2.new(0.5, 0.5)
		Center.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		Center.BackgroundTransparency = 1.000
		Center.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		BindEvent.Name = rares:RandomString();

		if not Menu.AutoFill then
			rares:ScrollSignal(Right,UIListLayout_3,'Y');
			rares:ScrollSignal(Center,UIListLayout_2,'Y');
			rares:ScrollSignal(Left,UIListLayout,'Y');
		else
		...
		Fatal.Signal.Event:Connect(function(Bool)
			if Bool then
				rares:CreateAnimation(MenuButton,0.5,{
					BackgroundTransparency = (MenuLiber.Visible and 0) or 1
				})

				rares:CreateAnimation(UIStroke,0.5,{
					Transparency = 0.950
				})

				rares:CreateAnimation(Left,0.3,{
					Position = UDim2.new(0.175, 0, 0.5, 0)
				})

				rares:CreateAnimation(Center,0.4,{
					Position = UDim2.new(0.5, 0, 0.5, 0)
				})

				rares:CreateAnimation(Right,0.5,{
					Position = UDim2.new(0.825, 0, 0.5, 0)
				})

				rares:CreateAnimation(Icon,0.5,{
					ImageTransparency = (MenuLiber.Visible and 0.15) or 0.5
				})

				rares:CreateAnimation(menu_name,0.5,{
					TextStrokeTransparency = 0.900,
					TextTransparency = (MenuLiber.Visible and 0.15) or 0.5
				})
			else
				rares:CreateAnimation(Left,0.5,{
					Position = UDim2.new(0.175, 0, 0.5, 1)
				})

				rares:CreateAnimation(Center,0.5,{
		...
		rares:CreateHover(MenuButton,function(bool)
		...
		rares:NewInput(MenuButton,function()
			for i,v in next , Fatal.Menus do
		...
		function MenuLib:AddPreview(Config: Preview)
			...
			PreviewName.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			PreviewName.FontFace = rares.FontSemiBold
			...
			MainBlock.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			local Toggle = function(v)
				if v then
					rares:CreateAnimation(Preview,0.5,{
						BackgroundTransparency = 0
					})

					rares:CreateAnimation(PreviewName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
					
					rares:CreateAnimation(Main,0.5,{
						BackgroundTransparency = 0
					})
					...
					rares:CreateAnimation(MainBlock,0.5,{
		...
		function MenuLib:AddListBox(Config : Listbox)
			...
			ListBox.Name = rares:RandomString()
			...
			ListBox.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			rares:AddDragBlacklist(ScrollingFrame);

			Elements.Name = rares:RandomString()
			...
			Elements.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			SearchBox.Name = rares:RandomString()
			SearchBox.Parent = Elements
			SearchBox.BackgroundColor3 = rares.Colors.Black
			...
			TextBox.BackgroundColor3 = rares.Colors.Black
			TextBox.BackgroundTransparency = 1.000
			TextBox.BorderColor3 = Color3.fromRGB(255, 255, 255)
			TextBox.BorderSizePixel = 0
			...
			local Toggle = function(v)
				if v then
					rares:CreateAnimation(ListBox,0.5,{
						BackgroundTransparency = 0
					})
					...
					rares:CreateAnimation(UIStroke_2,0.5,{
						Transparency = 0.75
					})

					rares:CreateAnimation(UIStroke_3,0.5,{
						Transparency = 0.75
					})

					rares:CreateAnimation(SearchBox,0.5,{
						BackgroundTransparency = 0
					})

					rares:CreateAnimation(TextBox,0.5,{
						TextTransparency = 0
					})
					...
					rares:CreateAnimation(ScrollingFrame,0.45,{
						BackgroundTransparency = 0,
					...
					table.foreach(ScrollingFrame:GetChildren(),function(i,v)
						if v:IsA('TextButton') then
							rares:CreateAnimation(v,0.45,{
								TextTransparency = 0.5
							});
						end;
					end);

					rares:CreateAnimation(ListName,0.5,{
				...
				Refresh = function()
					...
					for i,v in next , Config.Values do
						...
						if Config.Multi then
							...
							rares:CreateHover(bth,function(bool)
								if not Config.Default[v] then
									if bool then
										rares:CreateAnimation(bth,0.25,{
											TextTransparency = 0;
										})
									else
										rares:CreateAnimation(bth,0.25,{
			...
			if Config.Flag then
				rares.WindowFlags[rareswin][Config.Flag.."ListBox"] = res;
			end;
		...
		function MenuLib:AddSection(Config : Section)
			...
			local Toggle = function(v)
				if v then
					rares:CreateAnimation(Section,0.5,{
						BackgroundTransparency = 0
					})
					...
					rares:CreateAnimation(SectionName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
				else
			...
			Section.Name = rares:RandomString()
			...
			SectionName.Name = rares:RandomString()
			...
			SectionName.FontFace = rares.FontSemiBold;
			...
			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				...
				if not Menu.AutoFill then
					rares:CreateAnimation(Section,0.25,{
			...
			return rares:CreateElements(Elements,Elements.ZIndex,BindEvent,{
				Path = Menu.Name .. " > ".. Config.Name,
	...
	do
		InfoButton.Name = rares:RandomString()
		...
		InfoButton.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		SearchButton.Name = rares:RandomString()
		...
		SaveButton.Name = rares:RandomString()
		...
		rares:CreateHover(SaveButton,function(bool)
			if bool then
				rares:CreateAnimation(SaveButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				rares:CreateAnimation(SaveButton,0.5,{
		...
		rares:CreateHover(InfoButton,function(bool)
			if bool then
				rares:CreateAnimation(InfoButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				rares:CreateAnimation(InfoButton,0.5,{
	...
	do
		...
		rares:AddDragBlacklist(SearchFrame);
		...
		local SearchToggle = function(value)
			if value then
				...
				rares:CreateAnimation(SearchFrame,0.35,{
					Size = searchScale
				})
				...
				rares:CreateAnimation(SearchBox,0.35,{
					BackgroundTransparency = 0
				})
		...
		SearchFrame.Name = rares:RandomString()
		SearchFrame.Parent = rareswin;
		SearchFrame.AnchorPoint = Vector2.new(0, 1)
		SearchFrame.BackgroundColor3 = Color3.fromRGB(19, 19, 19)
		SearchFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		rares:CreateHover(SearchButton,function(bool)
			if bool then
				rares:CreateAnimation(SearchButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				rares:CreateAnimation(SearchButton,0.5,{
		...
		local get_button = function(Name,Path,TAB_WARP)
			...
			ResultFrame.Name = rares:RandomString()
			ResultFrame.Parent = ScrollingFrame;
			ResultFrame.BackgroundColor3 = rares.Colors.Black
			ResultFrame.BackgroundTransparency = 1.000
			ResultFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			FeatureName.Name = rares:RandomString()
			...
			FeatureName.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			FeatureName.FontFace = rares.FontSemiBold
			...
			FeaturePath.Name = rares:RandomString()
			...
			FeaturePath.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			FeaturePath.FontFace = rares.FontSemiBold
			...
			local button = rares:NewInput(ResultFrame);

			rares:CreateHover(button,function(bool)
				if bool then
					rares:CreateAnimation(ResultFrame,0.5,{
						BackgroundTransparency = 0
					})
				else
					rares:CreateAnimation(ResultFrame,0.5,{
		...
		SearchButton.MouseButton1Click:Connect(function()
			...
			table.foreach(Fatal.ElementContents,function(i,v)
				...
				SearchInformation[v.Path.." - "..rares:RandomString()] = {
					root = button,
					callback = function()
		...
		UserInputService.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				if not rares:IsMouseOverFrame(SearchFrame) then
					SearchToggle(false);
				end
			end
		end)
	...
	function Fatal:GetButton()
		...
		backpack.Name = "FATALITY"..rares:RandomString();
		...
		backpack.MouseEnter:Connect(function()
			rares:CreateAnimation(backpack,0.3,nil,{
				BackgroundColor3 = rares.Colors.Main,
...
function rares:Loader(Config: Loader)
	Config = Config or {};
	...
	Loader.Name = rares:RandomString()
	...
	rares:CreateAnimation(Blur,1,{
		Size = 60
	})

	rares:CreateAnimation(BlackFrame,0.5,{
	...
	local createText = function(TEXT)
		...
		local textsize = rares:GetTextSize(ASCII);

		ASCII.Size = UDim2.new(0, textsize.X + 100, 0, 50)
		LIT.Size = UDim2.new(0, (textsize.X * 2.5) + (UText[TEXT] or 0), 0, 100)

		UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 255, 255)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(132, 58, 58))}
	...
	do
		...
		StartText.Name = rares:RandomString()
		...
		StartText.BorderColor3 = Color3.fromRGB(255, 255, 255)
		...
		local textsize = rares:GetTextSize(StartText);
		...
		rares:CreateAnimation(StartText,0.45,{
			TextTransparency = 0
		})

		rares:CreateAnimation(UIScale,0.5,{
			Scale = Config.Scale;
		});

		task.wait(0.45);

		rares:CreateAnimation(StartText,0.35,{
		...
		for i,v in next , PosText do
			if i > 1 then
				rares:CreateAnimation(v.Text,0.65,{
					Position = UDim2.new(0.5,0,0.5,0);
					TextTransparency = 0
				})
			end;
		...
		rares:CreateAnimation(StartText,1.5,{
			TextTransparency = 1
		})

		for i,v in next , PosText do
			rares:CreateAnimation(v.Text,1.5,{
				TextTransparency = 1
			})
		end;

		rares:CreateAnimation(Blur,1.5,{
			Size = 0
		})

		rares:CreateAnimation(BlackFrame,1.5,{
			BackgroundTransparency = 1
		})
...
function rares:CreateNotifier(): Notifier
	if rares.__NOTIFIER_CACHE then return rares.__NOTIFIER_CACHE; end;

	local Notify = Instance.new("ScreenGui")
	local layout = Instance.new("Frame")
	local UIListLayout = Instance.new("UIListLayout")

	Notify.Name = rares:RandomString();
	Notify.Parent = CoreGui
	Notify.ResetOnSpawn = false
	Notify.ZIndexBehavior = Enum.ZIndexBehavior.Global
	Notify.IgnoreGuiInset = true;

	layout.Name = rares:RandomString();
	...
	layout.BorderColor3 = Color3.fromRGB(255, 255, 255)
	...
	local res = rares:CreateResponse({
		Notify = function(Config: Notify)
			...
			notify_block.Name = rares:RandomString()
			...
			notify_block.BorderColor3 = Color3.fromRGB(255, 255, 255)
			...
			Icon.BackgroundColor3 = rares.Colors.Main
			Icon.BackgroundTransparency = 1.000
			Icon.BorderColor3 = Color3.fromRGB(255, 255, 255)
			Icon.BorderSizePixel = 0
			Icon.Position = UDim2.new(0, 7, 0, 7)
			Icon.Size = UDim2.new(0, 18, 0, 18)
			Icon.Image = rares:GetIcon(Config.Icon);
			Icon.ImageColor3 = rares.Colors.Main
			...
			HeaderText.FontFace = rares.FontSemiBold
			HeaderText.Text = Config.Title
			HeaderText.TextColor3 = rares.Colors.Main
			HeaderText.TextSize = 13.000
			HeaderText.TextTransparency = 1
			HeaderText.TextXAlignment = Enum.TextXAlignment.Left

			BodyText.Name = rares:RandomString()
			...
			local updateScale = function()
				local TitleScale = rares:GetTextSize(HeaderText,Enum.Font.GothamBold);
				local ContentScale = rares:GetTextSize(BodyText,Enum.Font.GothamMedium);
				...
				rares:CreateAnimation(notify_block,0.35,{
					Position = UDim2.new(0.5, 0, 0, 0)
				});
			end;

			task.delay(0.1,function()
				rares:CreateAnimation(notify_block,0.25,{
					BackgroundTransparency = 0.1
				})

				rares:CreateAnimation(UIStroke,0.25,{
					Thickness = 2.500,
					Transparency = 0.900
				})

				rares:CreateAnimation(Icon,0.25,{
					ImageTransparency = 0
				})

				rares:CreateAnimation(HeaderText,0.25,{
					TextTransparency = 0.200
				})

				rares:CreateAnimation(BodyText,0.25,{
			...
			task.delay(Config.Duration + 0.25,function()
				...
				task.delay(0.35,function()
					rares:CreateAnimation(notify,0.5,{
						Size = UDim2.new(0,0,0,0)
					})
	...
	rares.__NOTIFIER_CACHE = res;

	return res;
end;

rares.FATALITY_PID = rares:RandomString();
